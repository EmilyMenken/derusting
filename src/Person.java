...
Person emilyPerson = new Person ("Emily", 20);

    // Create another instance of Person with a different name and age and
    // assign it to a different variable

Person dalePerson = new Person ("Dale", 65);

    // Print the first person

System.out.println(emilyPerson);

    // Print the second person

 System.out.println(dalePerson);

    // Get the name of the first person and store it in a local variable

    String emilyName = emilyPerson.name;
    ...
    int findBirthYear = emilyPerson.birthYear(currentYear);

    // In a separate statement, print the local variable holding the birth year.

System.out.println(emilyName+ " was born in " + findBirthYear);
